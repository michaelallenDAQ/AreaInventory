...
NOTE: For solvents, de always do pt source subtractions AFTER adding controls to the emissions estimates. We assume that our pt source emissions are already using the controls before they get phased in.
...
If reporting to NEI, DO NOT RUN THIS SECTION!!!
Utah currently defers to EPA for emission calculations for most fuel storage and transport SCCs, so we don't want to claim primacy when reporting these numbers. This section should only be run/added to the inventory when we are reporting for SIP projections or WEBSITE projections. For projections only.

```{r Storage and Transport (25-xx-xxx-xxx)}
...
#FOR ERIC: do the rest of these SCCs 
