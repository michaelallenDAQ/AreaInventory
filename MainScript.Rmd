...
#both of these are in kbbl
all_restoil <- projection_tables[["ResidualOil_Industrial"]] %>% filter(year == start_year)
pt_restoil <- projection_tables[["PtResidualOil_Industrial"]] %>% filter(year == start_year)

#this will be in kgal
area_resoil <- all_restoil %>% 
  mutate(unit = all_restoil$unit - pt_restoil$unit) %>%
  #zero out negative numbers
  mutate(unit = ifelse(unit<0, 0, unit)) %>%
  #convert from kbbl to kgal
  mutate(unit = unit*42)
...
#now that we have statewide area use of residual oil used on boilers, we distribute it to each county based on manufacturing employment
...
#add to final table and remove old variables
final_table <- rbind(final_table, projected_temp_table)
rm(scc, efs_table, all_restoil, pt_restoil, area_resoil, temp_table, projected_temp_table)
...
#now that we have statewide area use of wood used on boilers, we distribute it to each county based on county commercial employment
