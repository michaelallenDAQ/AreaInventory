...
---
Before you run this code, "git checkout main" in terminal, then "git pull" in terminal. This ensures you are working with the most up-to-date version of the code.
...
#I got this from a Cate Youatt email 2021/04/29. Then she messaged me May 18 and added
#10556. For the email attachment, look in references/20210520_OzoneSIPIDs.xlsx.

#Ozone_SIP_ids
target_ids <- c(10007,10008,10009,10028,10034,10096,10107,10119,10121,10122,
                10123,10129,10209,10210,10211,10237,10238,10259,10303,10311,
                10313,10327,10335,10346,10354,10355,10556,10571,10572,10627,
                10676,10706,10707,10716,10725,10790,10794,10892,11284,11386,
                11532,11767,12495,12512,12524,12929,12948,13031,13284,14010,
                14107)
...
#NEI_2017_ids 
target_ids <- c(10007,10008,10009,10028,10034,10096,10107,10119,10120,10121,
                10122,10123,10129,10156,10209,10210,10211,10237,10238,10259,
                10303,10311,10313,10327,10335,10346,10348,10354,10355,10369,
                10406,10414,10420,10423,10491,10555,10562,10565,10571,10572,
                10627,10676,10706,10707,10716,10725,10790,10794,10819,10823,
                10825,10843,10880,10892,10917,10922,11199,11234,11284,11386,
                11408,11532,11664,11767,11966,11977,12495,12512,12524,12825,
                12929,12948,13031,13043,13104,13144,13284,13315,14010,14107,
                14185,15438,15487)

#Web IDs is just every single site.
#Null means it there are no site ids, it will just look at everything
target_ids <- NULL
```


set up working directories to run the script with
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
#this is supposed to globally set our working directory. Sometimes this chunk doesn't successfully set the working directory. You'll be able to tell because R will give you an error like " Error: `path` does not exist: ‘20190502 NEI Workbook.xlsx’" when it tries to read a document. If you type in the console, the following two lines should have the same output:
    #Sys.getenv("HomeDir")
    #getwd()
#If that isn't the case, you will need to manually set the working directory. Type in the console:
#   setwd(Sys.getenv("HomeDir"))
#you can tell you need to reset the working directory if you have trouble running BaseCodes or BaseTables.
knitr::opts_knit$set(root.dir = Sys.getenv("HomeDir"))

#"HomeDir", for me, is "M:/My Drive/U_drive/main_workbook". I declared it in a file called ".Renviron". ".Renviron" contains two lines of text:
    #HomeDir=M:/My Drive/U_drive/main_workbook
    #GitDir=M:/My Drive/U_drive/GIT/AreaInventory/
#Look up Sys.getenv() online to learn more about this, but basically I declared two custom file pathways on RStudio, so when I want to read a file, it reads the right location.

```

set libraries, pull base functions and tables
set start/end year
```{r run this always}
library(readxl)
library(tidyverse)
...
I will also comment any relevant information
scc <- 2401055000

  3.2) Produce baseline emissions data for the SCC. This is either done from directly pulling emissions data from EPAs wagon wheel (WW) or NEI, OR we pull emission factors and throughputs and manually combine the EF and throughput to get a baseline. IF we don't just pull the baseline from the wagon wheel or NEI:

   3.2.1) Make an emission factor table for the SCC
   ...
   3.2.2) Make a throughput table for the SCC

   3.2.3) Produce the baseline year data. This is done by multiplying the EF table by the relevant throughput 'unit', which is typically population, employment data, or material combusted.

  3.3) Project baseline data. This is done by scaling the baseline data based on a given table of growth. Again, typically population, employment data, or material combusted. Once we have a pure, projected baseline of emissions to work with. We now apply controls and remove pt sources if relevant
   3.3.1) OPTIONAL: Pt source subtraction. Pull all potential Pt source SCCs that would overlap with the Area source SCC. We must project out the Pt source emissions and THEN subtract them away from the Area SCC. Ideally, we subtract uncontrolled pt source emissions from uncontrolled area source emissions, but in some cases we subtract controlled pt sources from our controlled area source emissions
   3.3.2) OPTIONAL: Apply controls. Ideally, we add controls after subtracting pt sources.
    
  3.4) Now that we have this new projected, controlled, pt_subtracted emission data, we add it to our final table and remove all of the old variables we used to generate emissions values.
  
########
First chunk, for projections. DO NOT RUN THIS SECTION FOR NEI REPORTING!!!
Utah currently defers to EPA for emission calculations for many SCC calculations. We do not want 
to claim primacy when reporting these numbers for the NEI, and instead will pull the data FROM the NEI and use them for projections. If we accidentally run and submit this section for NEI reporting, we will over-write whatever EPA comes up with, which would be bad. This section should only be run/added to the inventory when we are reporting for SIP projections or WEBSITE projections. For projections only.

There are no rules (and therefore, no controls) or pt source subtractions for these sccs.

```{r NEI Pulls, for SIPS and WEBSITE}

#25-XX-XXX-XXX: Storage and Transport of fuels

#declare an empty table to fill in with 25-XX-XXX-XXX SCCs. I can experiment with that 
#table before I merge it with final_table if I want
NEI_25xx <- vector()
source(paste0(Sys.getenv('GitDir'),'25XXXXXXXX_NEI_FuelStorageAndTransport.R'))
#merge the table with final and delete it.
final_table <- rbind(final_table, NEI_25xx)
rm(NEI_25xx)


#27-XX-XXX-XXX: Biogenic Emissions
#Biogenics are not human sourced. They are a huge portion of our statewide emissions, but 
#we don't look at them too closely, because they are not in our control.
NEI_27xx <- vector()
source(paste0(Sys.getenv('GitDir'),'27XXXXXXXX_NEI_Biogenics.R'))
#merge the table with final and delete it.
final_table <- rbind(final_table, NEI_27xx)
rm(NEI_27xx)


#28-XX-XXX-XXX: Agricultural processes, fires/wildfires, domestic/wild animals
...
#declare an empty table to fill in with 28-XX-XXX-XXX SCCs. I can experiment with that 
#table before I merge it with final_table if I want
NEI_28xx <- vector()
source(paste0(Sys.getenv('GitDir'),'28XXXXXXXX_NEI_AgAnimalsAndFires.R'))

#merge the table with final and delete it.
final_table <- rbind(final_table, NEI_28xx)
rm(NEI_28xx)

#done with NEI pulls
```

```{r 21-xxx-xxx-xxx: Stationary source fuel combustion}

#this section often distributes a statewide throughput to each county based on some other value like manufacturing employment, or commercial/government employment. I will set up those tables at the beginning of the chunk, and refer to them throughout:

man_emp <- projection_tables[["ManEmp"]] %>% filter(year==start_year)
#add a "fraction" column that consists of a single county's portion of employment vs the state.
man_emp <- man_emp %>% mutate(fraction = unit/sum(unit))

com_emp <- projection_tables[["CommercialEmployment"]] %>% filter(year==start_year)
#add a "fraction" column that consists of a single county's portion of employment vs the state.
com_emp <- com_emp %>% mutate(fraction = unit/sum(unit))
...
#INDUSTRIAL 
#bituminous/subbituminous coal combustion - all boiler types 
#EPA offers default calculations for this SCC, but we assume that all industrial combustion is happening on permitted sites, so after calculating this SCC... for now, we will reduce all values to 0 and then report that.
#ALL ZEROS

# Coal burned by point sources is assumed to be included in the industrial category except for Brigham Young University which is considered commercial per Michael Vanden Berg of the Utah Geological Survey Office. 							
#
#old note: It appears that the EIA may be overestimating the amount of coal burned in Utah by industrial sources (a similar overestimate by EIA for coal burned by electrical sources which are all point sources in Utah was also observed). Based on the above, EPA (Jenefer Snyder) were consulted and it was determined that it would be best to reflect the 2011 NEI and zero emissions for industrial nonpoint coal combustion as it appears that there are not any industrials/manufacturers burning coal that don't already have a permit (accounted for in Utah's point inventory) and we are just seeing an overestimate by EIA. See Greg Mortensen email to Jennifer Snyder on January 25, 2016 with final response on January 27, 2016."							

scc <- 2102002000
#the EFs for this SCC are given in the WW. The rest of our calculations are to get throughputs.
efs_table <- pull_efs_from_ww(scc, throughput_unit = "TON", all_primaries = TRUE)

#next we find the total area source industrial coal burnt in Utah. Area = TOTAL - PT SOURCE
total_coal <- projection_tables[["Coal_Industrial"]] %>% filter(year == start_year)
pt_coal <- projection_tables[["PtCoal_Industrial"]] %>% filter(year == start_year)

#set up area coal with the same format as total, but then subtract pt_coal from it
area_coal <- total_coal %>% 
  #subtract pt values from total.
  mutate(unit = total_coal$unit - pt_coal$unit) %>%
  #zero out negative numbers
  mutate(unit = ifelse(unit<0,0,unit))

#okay, now we have statewide industrial coal use. We need to distribute this coal on a per-county
#basis. For industrial coal, we distribute according to manufacturing employment
#units in tons
area_coal$unit <- area_coal$unit * man_emp$fraction

#now we have TONS of coal burned per county, and we have emission factors, in TPUPY (tons per unit per year). Combine the two tables
temp_table <- make_baseline(efs_table, area_coal)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["Coal_Industrial"]])

#control this to reduce to 0 lol. Assume all industrial coal combustion is on permits, assume EPA is overcounting
projected_temp_table <- add_manual_controls(projected_temp_table, scc, counties, 1900, 1901, 0)
#add to final_table
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(efs_table, total_coal, pt_coal, area_coal, temp_table, projected_temp_table)


#commercial/institutional
# Coal burned by point sources is assumed to be included in the industrial category except for Brigham Young University which is considered commercial per Michael Vanden Berg of the Utah Geological Survey Office. 							
#bituminous/subbituminous coal - all boiler types
#HISTORICALLY HAS BEEN ZEROS
scc <- 2103002000
#the EFs for this SCC are given in the WW. The rest of our calculations are to get throughputs.
efs_table <- pull_efs_from_ww(scc, throughput_unit = "TON", all_primaries = TRUE)

#next we find the total area source industrial coal burnt in Utah. Area = TOTAL - PT SOURCE
total_coal <- projection_tables[["Coal_Commercial"]] %>% filter(year == start_year)
pt_coal <- projection_tables[["PtCoal_Commercial"]] %>% filter(year == start_year)
#set up area coal with the same format as total, but then subtract pt_coal from it
area_coal <- total_coal %>%
  #subtract pt values from statewide values
  mutate(unit = total_coal$unit - pt_coal$unit) %>%
  #zero out negative numbers
  mutate(unit = ifelse(unit<0,0,unit))

#okay, now we have statewide industrial coal use. We need to distribute this coal on a per-county
#basis. For commercial/institutional coal, we distribute according to commercial employment
#units in tons
area_coal$unit <- area_coal$unit * com_emp$fraction

#now we have TONS of coal burned per county, and we have emission factors, in TPUPY (tons per unit per year). Combine the two tables
#again, this is probably zero
temp_table <- make_baseline(efs_table, area_coal)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["Coal_Commercial"]])
...
#Residential
# bituminous/subbituminous coal > all combustor types
# this scc could be calculated the same as 2103002000 and 2102002000 (we might need to pull in an extra table), but it's all zeros.
# WW says it's all zeros, too, so we're just gonna pull from WW.
scc <- 2104002000
temp_table <- pull_baseline_from_ww(scc)
#NEMO says SO2 EF is 31*S, where S is sulfur content. NEMO also gives S of 0 for Utah.
#If we want, we can add SO2 manually (below)
if(!"SO2" %in% temp_table$pollutant){
  cat("SO2 is not in the WW table for",scc,", so we are adding it manually. It should be 0 according to the NEMO")
  so2 <- make_simple_efs_table(scc,"SO2",0.0)
  so2 <- make_baseline(so2,projection_tables[["Steady"]])
  temp_table <- rbind(temp_table,so2)
  rm(so2)
}
temp_table_project <- project_baseline(temp_table, projection_tables[["Population"]])
#no pt subs, no controls
final_table <- merge_with_final_table(final_table,temp_table_project)
rm(scc, temp_table, temp_table_project)
...
# industrial
#combustion: kerosene > all boiler types
# It is assumed that industrial fuel combustion is included in the point source inventory. 2014 data indicated this to be the case
# If you look in the NEMO, the SO2 EF is "142S", where : "S indicates that the weight % of sulfur in the oil should be multiplied by the value given. For example, if the fuel is 1% sulfur, then S = 1"
#In Utah, S = 0.3%; therefore, 142 x 0.3 = 42.6 lbs/kgal. (I have not verified the 0.3% utah limit as of 20210511)									

#probably all zeros
#consider just pulling this one from WW, as it's often zero, and even if it isn't, it's small, EPA could take care of it.
scc <- 2102011000
#efs for kerosene can be pulled from WW. Throughput is per KGal.
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE)
#convert SO2 EF to reflect Utah's sulfur content. See notes on SCC
efs_table <- efs_table %>% mutate(TPUPY = ifelse(pollutant=="SO2",0.0213,TPUPY))

#throughputs are in KBBL!!!
total_kerosene <- projection_tables[["Kerosene_Industrial"]] %>% filter(year == start_year)
pt_kerosene <- projection_tables[["PtKerosene_Industrial"]] %>% filter(year == start_year)
#set up area kerosene with the same format as total, but then subtract pt_kerosene from it
area_kerosene <- total_kerosene %>% 
  mutate(unit = total_kerosene$unit - pt_kerosene$unit) %>%
  #zero out negative numbers
  mutate(unit = ifelse(unit<0,0,unit)) %>%
  #adjust KBBL throughput into KGal
  mutate(unit = unit * 42)

#okay, now distribute kerosene per county based on manufacturing employment
area_kerosene$unit <- area_kerosene$unit * man_emp$fraction
#since our units all match, let's combine these throughputs with our EFs
#again, probs all zeros
temp_table <- make_baseline(efs_table, area_kerosene)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["Kerosene_Industrial"]])

#no controls.
#add to final_table
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(efs_table, total_kerosene, pt_kerosene, area_kerosene, temp_table, projected_temp_table)


#commercial/institutional
#kerosene > all combustor types
# It is assumed that industrial fuel combustion is included in the point source inventory. 2014 data indicated this to be the case
# If you look in the NEMO, the SO2 EF is "142S", where : "S indicates that the weight % of sulfur in the oil should be multiplied by the value given. For example, if the fuel is 1% sulfur, then S = 1"
#In Utah, S = 0.3%; therefore, 142 x 0.3 = 42.6 lbs/kgal. (I have not verified the 0.3% utah limit as of 20210511)									
...
#efs for kerosene can be pulled from WW. Throughput is per KGal.
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE)
#this SCC fails to put in SO2 efs in the ww, so we have to add them manually
if(!"SO2" %in% efs_table$pollutant){
  cat("SO2 is not in the efs table for",scc,", so we are adding it manually")
  so2 <- make_simple_efs_table(scc,"SO2",0.0213)
  efs_table <- rbind(efs_table, so2)
  rm(so2)
} else{
  #convert SO2 EF to reflect Utah's sulfur content. See notes on SCC
  efs_table <-
    efs_table %>% mutate(TPUPY = ifelse(pollutant == "SO2", 0.0213, TPUPY))
}

#throughputs are in KBBL!!!
total_kerosene <- projection_tables[["Kerosene_Commercial"]] %>% filter(year == start_year)
pt_kerosene <- projection_tables[["PtKerosene_Commercial"]] %>% filter(year == start_year)
#set up area kerosene with the same format as total, but then subtract pt_kerosene from it
area_kerosene <- total_kerosene %>% 
  mutate(unit = total_kerosene$unit - pt_kerosene$unit) %>%
  #zero out negative numbers
...
#okay, now distribute kerosene per county based on commmercial employment
area_kerosene$unit <- area_kerosene$unit * com_emp$fraction
#since our units all match, let's combine these throughputs with our EFs
#again, probs all zeros
temp_table <- make_baseline(efs_table, area_kerosene)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["Kerosene_Commercial"]])
...
#residential
# kerosene > all heater types
#there are no pt subs for this one.
#EFs are given per kbbl instead of kgal?
# If you look in the NEMO, the SO2 EF is "142S", where : "S indicates that the weight % of sulfur in the oil should be multiplied by the value given. For example, if the fuel is 1% sulfur, then S = 1". 
#In Utah, S = 0.3%; therefore, 142 x 0.3 = 42.6 lbs/kgal. (I have not verified the 0.3% utah limit as of 20210511)									
#Since EFs are in bbl, and not gal, I will multiply the SO2 EF by 42
scc <- 2104011000
#efs for kerosene can be pulled from WW. Throughput is per Kbbl
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3BBL", all_primaries = TRUE)
#convert SO2 EF to reflect Utah's sulfur content. See notes on SCC. Add 42x for gal->bbl adjustment
efs_table <- efs_table %>% mutate(TPUPY = ifelse(pollutant=="SO2",0.0213*42,TPUPY))

#now we find residential kerosene used in the state, and distribute it per county based on number of houses that report using kerosene.
residential_kerosene <- projection_tables[["Kerosene_Residential"]] %>% filter(year == start_year)
housingKero <- projection_tables[["HousingKero"]] %>% filter(year==start_year)
#add a "fraction" column that consists of a single county's portion of employment vs the state.
housingKero <- housingKero %>% mutate(fraction = unit/sum(unit))

#okay, now distribute kerosene per county based on houses that use kerosene
residential_kerosene$unit <- residential_kerosene$unit * housingKero$fraction
#since our units all match, let's combine these throughputs with our EFs
temp_table <- make_baseline(efs_table, residential_kerosene)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["Kerosene_Residential"]])

#no controls.
#add to final_table
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(efs_table, residential_kerosene, housingKero, temp_table, projected_temp_table)
...
# Industrial and Commercial LPG consumption data is based on EIA's Utah SEDS csv file of "All Consumption Estimates" (see above) which is then adjusted to remove fuel potentially accounted for by mobile/other non-nonpoint categories and also adjusted for feedstock use (see Input file for details).
scc <- 2102007000
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE)

#get area source lpg statewide:
all_lpg <- projection_tables[["LPG_Industrial"]] %>% filter(year == start_year)
pt_lpg <- projection_tables[["PtLPG_Industrial"]] %>% filter(year == start_year)
#subtract pt from total to get area
area_lpg <- all_lpg
area_lpg$unit <- all_lpg$unit - pt_lpg$unit
#turn negative values to 0
area_lpg$unit[area_lpg$unit < 0] <- 0
#adjust kbbl into kgal
area_lpg$unit <- area_lpg$unit * 42

#now distribute statewide lpg use per county based on manufacturing employment
area_lpg$unit <- area_lpg$unit * man_emp$fraction

#both units are in kgal, so let's combine EFs with throughputs
temp_table <- make_baseline(efs_table, area_lpg)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["LPG_Industrial"]])
#no controls
final_table <- merge_with_final_table(final_table,projected_temp_table)
rm(scc, efs_table, all_lpg, pt_lpg, area_lpg, temp_table, projected_temp_table)


# commercial
# lpg, all combustor types
# Industrial and Commercial LPG consumption data is based on EIA's Utah SEDS csv file of "All Consumption Estimates" (see above) which is then adjusted to remove fuel potentially accounted for by mobile/other non-nonpoint categories and also adjusted for feedstock use (see Input file for details).
scc <- 2103007000
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE)

#get area source lpg statewide:
all_lpg <- projection_tables[["LPG_Commercial"]] %>% filter(year == start_year)
pt_lpg <- projection_tables[["PtLPG_Commercial"]] %>% filter(year == start_year)
#subtract pt from total to get area
area_lpg <- all_lpg %>%
  mutate(unit = all_lpg$unit - pt_lpg$unit) %>%
  #turn negative values to 0
  mutate(unit = ifelse(unit<0, 0, unit)) %>%
  #adjust kbbl into kgal
  mutate(unit = unit*42)

#now distribute statewide lpg use per county based on commercial/government employment
area_lpg$unit <- area_lpg$unit * com_emp$fraction

#both units are in kgal, so let's combine EFs with throughputs
temp_table <- make_baseline(efs_table, area_lpg)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["LPG_Commercial"]])
#no controls
final_table <- merge_with_final_table(final_table,projected_temp_table)
rm(scc, efs_table, all_lpg, pt_lpg, area_lpg, temp_table, projected_temp_table)
...
#residential
# lpg > all combustor types
# Residential LPG consumption data is from EIA's Utah SEDS csv file of "All Consumption Estimates" which can be downloaded and combined with a "Codes and Descriptions" file, both available at: http://www.eia.gov/state/seds/seds-data-complete.cfm?sid=US#stateSelection (see Input file for details).									
scc <- 2104007000
#for some reason WW throughput is in kbbl, not kgal
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3BBL", all_primaries = TRUE)

#pull statewide LPG usage for residents (in kbbl)
residential_lpg <- projection_tables[["LPG_Residential"]] %>% filter(year == start_year)
#pull number of houses per county that use LPG
housingLPG <- projection_tables[["HousingLPG"]] %>% filter(year == start_year)
housingLPG <- housingLPG %>% mutate(fraction = unit/sum(unit))

#distribute statewide LPG per county
residential_lpg$unit <- residential_lpg$unit * housingLPG$fraction

#since EF and throughput are both in KBBL, let's combine them to form a baseline
temp_table <- make_baseline(efs_table, residential_lpg)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["LPG_Residential"]])

#no controls.
#add to final_table
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, residential_lpg, housingLPG, temp_table, projected_temp_table)
...
#industrial
#nat gas > total: boilers and IC engines
# Future improvement: Look for means of adjusting Industrial/Commercial combusted natural gas MMCF to remove mobile (CNG cars) use (possibly accounted for by mobile).										
#(10/16/17) Ammonia emission factor was changed to zero based on industry feedback (Frank Stanonick, Air Conditioning, Heating & Refrigeration Institute call with Joel Karmazyn)  which also reflects that AP42 has no emission factor for ammonia. - I AM IGNORING THIS COMMENT 2021/05/11. EPA does give documentation for their ammonia EF., and we also never reduced the EF to 0 on the workbook.
scc <- 2102006000
#most up-to-date emission factors can be pulled from WW
#note that units are in MMCF, or million cubit feet
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E6FT3", all_primaries = TRUE)

#now that we have EFs, get throughput. Start by pulling the Electrical/Industrial/Commercial NG use for all of Utah. Basically all of the nonresidential NG use in Utah. Units MMCF
nonresNG <- projection_tables[["NonResNGMMCF_Utah"]] %>% filter(year == start_year)
# Questar's Dave Landward (dave.landward@questar.com or 801-324-5722) provided Questar's 2014 usage consumption proportions by sector (1/27/16 email):
#   Residential:  36%
#   Commercial:   22%
#   Industrial:   19%
#   Electric:     23%
#Since the table we just pulled was for Electrical/Industrial/Commercial NG, we can ignore the residential portion of distribution. The industrial portion would then be (19)/(19+22+23)
industrialNG <- nonresNG %>% mutate(unit = unit*(19)/(19+22+23))
#these percentages could be revisited for accuracy in the future.

#now we pull all pt source industrial NG use (MMCF)
PtindustrialNG <- projection_tables[["PtNG_Industrial"]] %>% filter(year == start_year)

#now subtract pt sources from the total NG use to get area source NG
#might make things 0
AreaindustrialNG <- industrialNG %>% 
  mutate(unit = unit - PtindustrialNG$unit) %>%
  #adjust negative values to be 0
  mutate(unit = ifelse(unit<0,0,unit))

#the final subtraction we do for NG is NG used for "Feedstock".
#I (Michael) don't understand this section, but am copying it from the workbook:
# For commercial and industrial sources, Questar's natural gas non-residential consumption data are adjusted for feedstock use. Per Dave Landward (conversation on 1/26/16), Questar Gas does not track end-use of delivered gas, thus, natural gas may be used as a feedstock by commercial/industrial sources in Utah.

#EPA Feedstock adjustment (proportion used for feedstock - divide feedstock use by total industrial nonfuel use)											
#Based on EIA Data: MECS (Manufacturing Energy Consumption Survey) http://www.eia.gov/consumption/manufacturing/data/2010/
#Divide West Census Region feedstock use (Table 2.1) by total industrial nonfuel use (Table 1.1)
#Data only available every four years.
#MECS Year        2010
#Feedstock use    14    *Note: EPA/NOMAD uses 0.25 units for all MECS <0.5. 
#Nonfuel use      611
#So what this means is 14/611 units of gas are used for feedstock? So we want to remove that from our combustion throughput.
AreaindustrialNG <- AreaindustrialNG %>% mutate(unit = unit*(1-(14/611)))

#okay, so now we have true statewide industrial NG usage for area source. We now distribute it to each county based on NG use in each county. The data we have on natural gas usage per county is measured in Decatherms (not in MMCF), so that is how we split things up.
nonresNGDecatherm <- projection_tables[["NonResNGDecatherm"]] %>% filter(year == start_year) %>%
  mutate(fraction = unit/sum(unit))
AreaindustrialNG <- AreaindustrialNG %>% mutate(unit = unit*nonresNGDecatherm$fraction)

#now we have area NG use by county in MMCF, and EFs in MMCF. Combine them
temp_table <- make_baseline(efs_table, AreaindustrialNG)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["NonResNGMMCF_Utah"]])
#add to final_table and delete residual variables
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, nonresNG, industrialNG, PtindustrialNG, AreaindustrialNG, nonresNGDecatherm, temp_table, projected_temp_table)
...
#   Electric:     23%
#Since the table we just pulled was for Electrical/Industrial/Commercial NG, we can ignore the residential portion of distribution. The industrial portion would then be (19)/(19+22+23)
commercialNG <- nonresNG %>% mutate(unit = unit*(22)/(19+22+23))
#these percentages could be revisited for accuracy in the future.

#now we pull all pt source industrial NG use (MMCF)
PtcommercialNG <- projection_tables[["PtNG_Commercial"]] %>% filter(year == start_year)

#now subtract pt sources from the total NG use to get area source NG
AreacommercialNG <- commercialNG %>% 
  mutate(unit = unit - PtcommercialNG$unit) %>%
  #adjust negative values to be 0
  mutate(unit = ifelse(unit<0,0,unit))
...
#Nonfuel use      611
#So what this means is 14/611 units of gas are used for feedstock? So we want to remove that from our combustion throughput.
AreacommercialNG <- AreacommercialNG %>% mutate(unit = unit*(1-(14/611)))

#okay, so now we have true statewide industrial NG usage for area source. We now distribute it to each county based on NG use in each county. The data we have on natural gas usage per county is measured in Decatherms (not in MMCF), so that is how we split things up.
nonresNGDecatherm <- projection_tables[["NonResNGDecatherm"]] %>% filter(year == start_year) %>%
  mutate(fraction = unit/sum(unit))
AreacommercialNG <- AreacommercialNG %>% mutate(unit = unit*nonresNGDecatherm$fraction)

#now we have area NG use by county in MMCF, and EFs in MMCF. Combine them
temp_table <- make_baseline(efs_table, AreacommercialNG)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["NonResNGMMCF_Utah"]])
#add controls
controlled_projected <- add_controls(projected_temp_table)
#add to final_table and delete residual variables
final_table <- merge_with_final_table(final_table, controlled_projected)
rm(scc, efs_table, nonresNG, commercialNG, PtcommercialNG, AreacommercialNG, nonresNGDecatherm, temp_table, projected_temp_table, controlled_projected)




#residential
#nat gas > all combustor types
scc <- 2104006000
#first, pull EFs from WW
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E6FT3", all_primaries = TRUE)

#now pull statewide residential NG use, in MMCF
resNGMMCF <- projection_tables[["ResNGMMCF_Utah"]] %>% filter(year == start_year)

#now distribute MMCF per county based on residential decatherm usage per county
resNGDecatherm <- projection_tables[["ResNGDecatherm"]] %>%
  filter(year == start_year) %>%
  mutate(fraction = unit/sum(unit))
resNGMMCF$unit <- resNGMMCF$unit * resNGDecatherm$fraction

#now apply emission factors, all units in MMCF
temp_table <- make_baseline(efs_table, resNGMMCF)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["ResNGMMCF_Utah"]])
#add controls
controlled_projected <- add_controls(projected_temp_table)

final_table <- merge_with_final_table(final_table, controlled_projected)
rm(scc, efs_table, resNGMMCF, resNGDecatherm, temp_table, projected_temp_table, controlled_projected)
...
#5) Residential oil consumption data is from EIA's Utah SEDS csv file of "All Consumption Estimates" which can be downloaded and combined with a "Codes and Descriptions" file, both available at: http://www.eia.gov/state/seds/seds-data-complete.cfm?sid=US#stateSelection (see Input file for details).									
scc <- 2104004000
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE) %>%
# If you look in the NEMO, the SO2 EF is "142S", where : "S indicates that the weight % of sulfur in the oil should be multiplied by the value given. For example, if the fuel is 1% sulfur, then S = 1"
#In Utah, S = 0.3%; therefore, 142 x 0.3 = 42.6 lbs/kgal. (I have not verified the 0.3% utah limit as of 20210511)									
  mutate(TPUPY = ifelse(pollutant =="SO2",42.6/2000,TPUPY))

#now get state throughput in kgal
res_distillateOil <- projection_tables[["DistillateOil_Residential"]] %>% 
  filter(year == start_year) %>%
  #adjust units from kbbl to kgal
  mutate(unit = unit*42)

#now distribute the statewide number to each county based on number of houses that report using KEROSENE? interesting... I'll just leave it.
housing_distillate <- projection_tables[["HousingKero"]] %>%
  filter(year == start_year) %>%
  mutate(fraction = unit/sum(unit))
res_distillateOil$unit <- res_distillateOil$unit * housing_distillate$fraction

#now we have throughput and EF in kgal. Combine them.
temp_table <- make_baseline(efs_table, res_distillateOil)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["DistillateOil_Residential"]])
#no controls
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, res_distillateOil, housing_distillate, temp_table, projected_temp_table)


#industrial
#distillate oil > all boiler types

scc <- 2102004001
#pull emission factors from ww in kgal
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE) %>%
# If you look in the NEMO, the SO2 EF is "142S", where : "S indicates that the weight % of sulfur in the oil should be multiplied by the value given. For example, if the fuel is 1% sulfur, then S = 1"
#In Utah, S = 0.3%; therefore, 142 x 0.3 = 42.6 lbs/kgal. (I have not verified the 0.3% utah limit as of 20210511)									
  mutate(TPUPY = ifelse(pollutant =="SO2",42.6/2000,TPUPY))

#both of these are in kbbl
all_distoil <- projection_tables[["DistillateOil_Industrial"]] %>% filter(year == start_year)
pt_distoil <- projection_tables[["PtDistillateOil_Industrial"]] %>% filter(year == start_year)

#this will be in kgal
area_distoil <- all_distoil %>% 
  mutate(unit = all_distoil$unit - pt_distoil$unit) %>%
  #zero out negative numbers
  mutate(unit = ifelse(unit<0, 0, unit)) %>%
  #convert from kbbl to kgal
  mutate(unit = unit*42)
# In the 2014 ICI Tool, EPA/NOMAD changed their default Engine/Boiler split from 50/50 to: Industrial - 60% Boiler, 40% Engine This was based on national figures from MECS for the industrial split (https://www.eia.gov/consumption/manufacturing/data/2010/pdf/Table5_5.pdf) and CBECS for the commercial split (http://www.eia.gov/consumption/commercial/data/2003/pdf/e09.pdf).

#now distribute area source distillate oil use to only the portion used on boilers
#this distribution was done incorrectly in the old workbook.
area_distoil <- area_distoil %>% mutate(unit = unit*0.60)

#now that we have statewide area use of distillate oil used on boilers, we distribute it to each county based on manufacturing employment
area_distoil <- area_distoil %>% mutate(unit = unit*man_emp$fraction)

#now we have oil used per county, combine it with our emission factors
temp_table <- make_baseline(efs_table, area_distoil)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["DistillateOil_Industrial"]])
#no controls

#add to final table and remove old variables
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, all_distoil, pt_distoil, area_distoil, temp_table, projected_temp_table)
...
#now distribute area source distillate oil use to only the portion used on engines
#this distribution was done incorrectly in the old workbook.
area_distoil <- area_distoil %>% mutate(unit = unit*0.40)

#now that we have statewide area use of distillate oil used on engines, we distribute it to each county based on manufacturing employment
area_distoil <- area_distoil %>% mutate(unit = unit*man_emp$fraction)
...
#both of these are in kbbl
all_distoil <- projection_tables[["DistillateOil_Commercial"]] %>% filter(year == start_year)
pt_distoil <- projection_tables[["PtDistillateOil_Commercial"]] %>% filter(year == start_year)
...
area_distoil <- all_distoil %>% 
  ...
  #convert from kbbl to kgal
  mutate(unit = unit*42)
# In the 2014 ICI Tool, EPA/NOMAD changed their default Engine/Boiler split from 50/50 to: Commercial - 95% Boiler, 5% Engine. This was based on national figures from MECS for the industrial split (https://www.eia.gov/consumption/manufacturing/data/2010/pdf/Table5_5.pdf) and CBECS for the commercial split (http://www.eia.gov/consumption/commercial/data/2003/pdf/e09.pdf).
#now distribute area source distillate oil use to only the portion used on boilers
area_distoil <- area_distoil %>% mutate(unit = unit*0.95)

#now that we have statewide area use of distillate oil used on boilers, we distribute it to each county based on commercial employment
area_distoil <- area_distoil %>% mutate(unit = unit*com_emp$fraction)

#now we have oil used per county, combine it with our emission factors
temp_table <- make_baseline(efs_table, area_distoil)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["DistillateOil_Commercial"]])
#no controls
...
# In the 2014 ICI Tool, EPA/NOMAD changed their default Engine/Boiler split from 50/50 to: Commercial - 95% Boiler, 5% Engine. This was based on national figures from MECS for the industrial split (https://www.eia.gov/consumption/manufacturing/data/2010/pdf/Table5_5.pdf) and CBECS for the commercial split (http://www.eia.gov/consumption/commercial/data/2003/pdf/e09.pdf).
#now distribute area source distillate oil use to only the portion used on engines
area_distoil <- area_distoil %>% mutate(unit = unit*0.05)

#now that we have statewide area use of distillate oil used on engines, we distribute it to each county based on commercial employment
area_distoil <- area_distoil %>% mutate(unit = unit*com_emp$fraction)
...
#pull emission factors from ww in kgal
efs_table <-
  pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE) %>%
  #   Industrial & commercial residual oil SO2 Emission factor is adjusted as follows:
  #   SO2=157(S), therefore SO2=157(S) where "S" is % Sulfur by wt. In Utah, S = 0.3%; therefore, 157 x 0.3 = 47.1 lbs/kgal.
  #   Industrial PM-FIL emission factors are calculated as follows: 
  #   PM10-FIL is 7.17*(1.12*S+0.37), thus  7.17*(1.12*0.3+0.37) = 5.06202. 
  #   PM2.5-FIL is 4.67*(1.12*S+0.37), thus  4.67*(1.12*.3+0.37) = 3.29702.
  mutate(TPUPY = ifelse(pollutant =="SO2",47.1/2000,TPUPY)) %>%
  mutate(TPUPY = ifelse(pollutant =="PM10-FIL",5.06202/2000,TPUPY)) %>%
  mutate(TPUPY = ifelse(pollutant =="PM25-FIL",3.29702/2000,TPUPY)) 

#both of these are in kbbl
all_resoil <- projection_tables[["ResidualOil_Industrial"]] %>% filter(year == start_year)
pt_resoil <- projection_tables[["PtResidualOil_Industrial"]] %>% filter(year == start_year)

#this will be in kgal
area_resoil <- all_resoil %>% 
  mutate(unit = all_resoil$unit - pt_resoil$unit) %>%
  #zero out negative numbers
  mutate(unit = ifelse(unit<0, 0, unit)) %>%
  #convert from kbbl to kgal
  mutate(unit = unit*42)
# In the 2014 ICI Tool, EPA/NOMAD changed their default Engine/Boiler split from 50/50 to: Industrial - 60% Boiler, 40% Engine. This was based on national figures from MECS for the industrial split (https://www.eia.gov/consumption/manufacturing/data/2010/pdf/Table5_5.pdf) and CBECS for the commercial split (http://www.eia.gov/consumption/commercial/data/2003/pdf/e09.pdf).
#now distribute area source residual oil use to only the portion used on boilers
#this distribution was done incorrectly in the old workbook.
area_resoil <- area_resoil %>% mutate(unit = unit*0.60)

#now that we have statewide area use of industrial residual oil used on boilers, we distribute it to each county based on manufacturing employment
area_resoil <- area_resoil %>% mutate(unit = unit*man_emp$fraction)

#now we have oil used per county, combine it with our emission factors
temp_table <- make_baseline(efs_table, area_resoil)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["ResidualOil_Industrial"]])
#no controls
#add to final table and remove old variables
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, all_resoil, pt_resoil, area_resoil, temp_table, projected_temp_table)


#commercial/institutional
#residual oil > all boilers
scc <- 2103005000
#pull emission factors from ww in kgal
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E3GAL", all_primaries = TRUE) %>%
  #   Commercial PM-FIL emission factors are calculated as follows: 
  #   SO2=157(S), therefore SO2=157(S) where "S" is % Sulfur by wt. In Utah, S = 0.3%; therefore, 157 x 0.3 = 47.1 lbs/kgal.
  #   PM10-FIL is 5.17*(1.12*S+0.37), thus  5.17*(1.12*0.3+0.37) =3.65002. 
  #   PM2.5-FIL is 1.92*(1.12*S+0.37), thus 1.92*(1.12*.3+0.37) = 1.35552.							
  mutate(TPUPY = ifelse(pollutant =="SO2",47.1/2000,TPUPY)) %>%
  mutate(TPUPY = ifelse(pollutant =="PM10-FIL",3.65002/2000,TPUPY)) %>%
  mutate(TPUPY = ifelse(pollutant =="PM25-FIL",1.35552/2000,TPUPY)) 

#both of these are in kbbl
all_resoil <- projection_tables[["ResidualOil_Commercial"]] %>% filter(year == start_year)
pt_resoil <- projection_tables[["PtResidualOil_Commercial"]] %>% filter(year == start_year)
...
area_resoil <- all_resoil %>% 
  ...
  #convert from kbbl to kgal
  mutate(unit = unit*42)
# In the 2014 ICI Tool, EPA/NOMAD changed their default Engine/Boiler split from 50/50 to: Industrial - 60% Boiler, 40% Engine and Commercial - 95% Boiler, 5% Engine. This was based on national figures from MECS for the industrial split (https://www.eia.gov/consumption/manufacturing/data/2010/pdf/Table5_5.pdf) and CBECS for the commercial split (http://www.eia.gov/consumption/commercial/data/2003/pdf/e09.pdf).
#now distribute area source residual oil use to only the portion used on boilers
#this distribution was done incorrectly in the old workbook.
area_resoil <- area_resoil %>% mutate(unit = unit*0.95)

#now that we have statewide area use of residual oil used on boilers, we distribute it to each county based on commercial and government employment
area_resoil <- area_resoil %>% mutate(unit = unit*com_emp$fraction)

#now we have oil used per county, combine it with our emission factors
temp_table <- make_baseline(efs_table, area_resoil)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["ResidualOil_Commercial"]])
#no controls
...
#Stationary Source Fuel Combustion > 
#Commercial > Wood > All Boiler Types
scc <- 2103008000
#pull emission factors from ww in MMBTU
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E6BTU", all_primaries = TRUE) 

#pull BTU of wood combusted in Utah in billion BTU
UtahWood <- projection_tables[["WoodBTU_Commercial"]] %>% filter(year == start_year) %>% 
  #Table is pulled in Billion BTU, switch to MMBTU
  mutate(unit=unit*1000)

#now that we have statewide area use of wood used on boilers, we distribute it to each county based on county commercial employment. We do not split this into boilers/IC engines, and instead assume all wood is used for boilers.
area_Wood <- UtahWood %>% mutate(unit = unit*com_emp$fraction)

#now we have wood used per county, combine it with our emission factors
temp_table <- make_baseline(efs_table, area_Wood)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["WoodBTU_Commercial"]])
#no controls

#add to final table and remove old variables
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, UtahWood, area_Wood, temp_table, projected_temp_table)


#Industrial wood > all boilers
scc <- 2102008000
#pull EFs in MMBTU
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E6BTU", all_primaries = TRUE)

#pull statewide industrial wood use in Billion BTU
UtahWood <- projection_tables[["WoodBTU_Industrial"]] %>% filter(year == start_year) %>%
  #convert from billion BTU to MMBTU
  mutate(unit = unit*1000)
#distribute statewide wood use per county based on manufacturing employment
area_wood <- UtahWood %>% mutate(unit = unit*man_emp$fraction)

#now we have wood burned in each county, combine it with EFs
temp_table <- make_baseline(efs_table, area_wood)
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["WoodBTU_Industrial"]])
#no controls
...
rm(man_emp, com_emp)
#done with 21-xx sccs

```

```{r 22-XX, 26-XX, unpaved roads, waste disposal}
...
final_table <- merge_with_final_table(final_table, temp_table)


#Opening burning of leafs in people's yards
...
scc <- 2620030000
#efs were pulled from looking at permitted landfills. We took their throughputs and emissions and back-calculated to find the throughputs. For documentation, look in references/2620030000_Landfills.xlsx
pollutants <- c("VOC", "NH3", "PM10-PRI", "PM10-FIL", "PM25-PRI", "PM25-FIL")
TPPPY <- c(5.07E-5, 5.86E-7, 1.11E-5, 1.11E-5, 4.03E-6, 4.03E-6)
efs_table <- make_simple_efs_table(scc, pollutants, TPPPY)

#LandfillTons is the tonnage of waste that has gone through landfills that aren't counted as "Major point sources" for whichever source we are reporting to (NEI/WEBSITE/SIP). That is the activity data we combine with the EFs.
temp_table <- make_baseline(efs_table, projection_tables[["LandfillTons"]])
#projects based on population
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["Population"]])
#controls
#Utah Rule R307-221 applied controls to waste disposal through flaring and gas capture requirements. The control is already accounted for in the above emission factor as the factor is an average of post-control emissions from landfills.										
#merge with final table and delete old variables
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, pollutants, TPPPY, efs_table, temp_table, projected_temp_table)


#waste disposal, treatment, and recovery
#wastewater treatment > public owned > total processed
# Flow rate was calculated based on publicly owned treatment works (POTW) plant data (see relevent appendix in the Input Appendicies) provided by the Utah Division of Water Quality - Kim Shelley 801-536-4385 or kshelley@utah.gov							
# Point source flows are subtracted from flow rates prior to emission calculation							
# For our purposes, there is no difference between a POTW, WWTP, a "Sewer Facility," and a "Water Reclamation Facility."							
scc <- 2630020000
efs_table <- pull_efs_from_ww(scc, throughput_unit = "E6GAL", all_primaries = TRUE)

#now that we have EFs, we get nonpoint sewer flow. We calculate nonpoint flow by taking statewide flow, and then subtracting away the pt source flow
#units are in million-gallons, or E6GAL
sewerFlow <- projection_tables[["SewerFlow"]] %>% filter(year == start_year)
ptSewer <- projection_tables[["PtSewer"]] %>% filter(year == start_year)

#get area flow by subtracting pt flow from total flow
areaSewer <- sewerFlow %>% mutate(unit = sewerFlow$unit - ptSewer$unit) %>%
  mutate(unit = ifelse(unit<0,0,unit))

#now we have area efs and area throughput, combine them
temp_table <- make_baseline(efs_table, areaSewer)
projected_temp_table <- project_baseline(temp_table, projection_table = projection_tables[["Population"]])
#no controls

#merge with final_table, 
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, efs_table, sewerFlow, ptSewer, areaSewer, temp_table, projected_temp_table)


#waste disposal, treatment, and recovery
#composting > 100% green waste (e.g. residential/municipal yard wastes) > all processes
# Basis for calculations and emission factors are from EPA/NOMAD calculations (ftp://ftp.epa.gov/EmisInventory/2014/doc/nonpoint/). However, rather than distributing emissions based on landfill employment, population was used because landfill employment did not appear to be accurate and population would provide a better distribution. 								
# Likely Double Count! Yard waste generated is based on the same number (0.35 lbs/person/day) used in "Open Burning & Incineation" for leaf and brush waste and not all yard waste is composted, a lot will be placed in garbage and taken to the regular landfill and accounted for in landfill emissions. Thus only a fraction of yard waste should be considered composted. Currently, that is not the case.								

scc <- 2680003000
#ideally would pull EFs from WW, but there are two "primary" pollutants in the wagon wheel EFs, and my code can't handle that. Unfortunately I will declare them manually for now. I will also skip the secondary emissions/HAPs.
#ultimately we should pull these from WW
pollutants <- c("VOC", "NH3")
TPPPY <- c(4.67/2000, 0.66/2000)
#these EFs are in lb/TON of material composted.
efs_table <- make_simple_efs_table(scc, pollutants, TPPPY)

#now for throughput. We want tons of green waste composted per county.
#we assume 0.354004185 lb of yard waste is produced per person per day.
yardWaste <- projection_tables[["Population"]] %>% filter(year == start_year) %>%
  #             person  lb/person-day      day/yr   ton/lb
  mutate(unit = unit *   0.354004185*       365     /2000)
#units of "yardWaste" are now in tons of yard waste per year

#now we have efs in tons/ton of yard waste, and we have tons of yard waste. Combine them for baseline
temp_table <- make_baseline(efs_table, yardWaste)
projected_temp_table <- project_baseline(temp_table, projection_table = projection_tables[["Population"]])
#no controls
#add to final_table
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, pollutants, TPPPY, efs_table, yardWaste, temp_table, projected_temp_table)

#done with 22-xx, 26-xx sccs
```

```{r 23-xx-xxx-xxx: food, construction, mining }
...
# Rule 370-303 has a 76% reduction in emissions for VOC and PM25.
...
temp_table_subtracted <- subtract_pt_sources(temp_table, pt_table)
#no controls
final_table <- merge_with_final_table(final_table, temp_table_subtracted)
rm(scc, pollutants, TPPPY, efs_table, temp_table, pt_sccs, pt_table, temp_table_subtracted)
...
# add control
# since its time dependent and we're past the phaseinendyear (2014), no controls will be added. But if we backproject to 2012, emissions will increase a bit for years prior to 2015.
temp_table <- add_controls(temp_table)

# merge with final table
final_table <- merge_with_final_table(final_table,temp_table)
rm( scc, pt_sccs, real_table, pollutants, TPPPY, efs_table, zeros_table, temp_table)




#construction: SIC 15-17 > residential > totals
#WW has calculations on this, but we pull our own housing construction data, so we do things ourselves.
scc <- 2311010000
#we will define EFs independently for each type of construction
#each county has its own adjustment factor to its EF, based on statewide precipitation, but also
#county average silt percent. These numbers/equations are from the NEMO/WW inputs
utah_precipitation_evaporation_value <- 29.6
county_silt_percentages <- 
  data.frame(county = counties, 
             silt_percent = c(42.94, 45.26, 49.25, 41.15, 39.2, 51.23, 28.63, 24.18, 30.57,
                              18, 33.4, 37.66, 29.1, 36.12, 38.52, 38.19, 38.5, 46.82, 21.04,
                              45.12, 36.9, 37.53, 43.69, 37.35, 41.22, 43.18, 24.59, 18.72, 39.8)) %>%
  mutate(adjustment_factor = 
           24/utah_precipitation_evaporation_value * 
           silt_percent/100/0.09)


#now I need to pull the acre-months per year for each county.
# for residential construction, there are several buildings that each have different acreages and timescales of construction. 
#we get our calculation assumptions from the NOMAD/NEMO
#   Apartments
#   Single-family units
#   Two-family units

      #apartments
#we assume apartment construction disturbs 0.5 acres, and that it lasts 12 months.
apartments <- projection_tables[["Apartments"]] %>% filter(year == start_year) %>%
  #we pull new units constructed, so multiply by (0.5*12) to get acre months
  mutate(unit = unit*0.5*12)

#EFs for apartment construction are in the NOMAD - you will have to check that these are up to date
pollutants <- c('PM10-PRI','PM25-PRI','PM10-FIL','PM25-FIL')
#given in TON per ACRE-MONTH. PM25 is assumed to be 10% of PM10
TPPPY <-c(0.11, 0.11*0.1, 0.11, 0.11*0.1)
efs_table <- make_simple_efs_table(scc,pollutants,TPPPY)
#now I need to adjust the EF for each county based on the county_silt_percentages adjustment factors
efs_table <- left_join(efs_table, county_silt_percentages, by="county") %>%
  mutate(TPUPY = TPUPY * adjustment_factor) %>%
  #drop unwanted columns
  select(-c("silt_percent", "adjustment_factor"))
#make emissions table for apartments, then later combine it with the rest
apartments_table <- make_baseline(efs_table, apartments)


      #single-family units
#we assume single-family units take up 1/4 of an acre and last 6 months
#single family units with basements have different emission factors than no basements
basement_pct <- 0.2681
single_units <- projection_tables[["SingleUnits"]] %>% filter(year == start_year) %>%
  #we pull new units constructed, so multiply by (0.25*6) to get acre months
  mutate(unit = unit*0.25*6)

single_nobasement <- single_units %>% mutate(unit = unit*(1-basement_pct))
single_basement <- single_units %>% mutate(unit = unit*basement_pct)

#EFs for single unit construction are in the NOMAD - you will have to check that these are up to date
#no basements:
pollutants <- c('PM10-PRI','PM25-PRI','PM10-FIL','PM25-FIL')
#given in TON per ACRE-MONTH. PM25 is assumed to be 10% of PM10
TPPPY <-c(0.032, 0.032*0.1, 0.032, 0.032*0.1)
efs_table <- make_simple_efs_table(scc,pollutants,TPPPY)
#now I need to adjust the EF for each county based on the county_silt_percentages adjustment factors
efs_table <- left_join(efs_table, county_silt_percentages, by="county") %>%
  mutate(TPUPY = TPUPY * adjustment_factor) %>%
  #drop unwanted columns
  select(-c("silt_percent", "adjustment_factor"))
#make emissions table for no-basement 1-units, then later combine it with the rest
nobasement <- make_baseline(efs_table, single_nobasement)

#yes basements:
#we also have to calculate emissions for the dirt displaced:
#emissions are dust per 1,000 cubic yards of dirt displaced.
#651.9 cubic yards per basement
kcubic_yards <- 0.651852
basement_dusts <- projection_tables[["SingleUnits"]] %>%
  filter(year == start_year) %>%
  mutate(unit = unit*basement_pct) %>%
  mutate(unit = unit*kcubic_yards)

pollutants <- c('PM10-PRI','PM25-PRI','PM10-FIL','PM25-FIL')
#given in TON per ACRE-MONTH. PM25 is assumed to be 10% of PM10
TPPPY <-c(0.059, 0.059*0.1, 0.059, 0.059*0.1)
efs_table <- make_simple_efs_table(scc,pollutants,TPPPY)
#now I need to adjust the EF for each county based on the county_silt_percentages adjustment factors
efs_table <- left_join(efs_table, county_silt_percentages, by="county") %>%
  mutate(TPUPY = TPUPY * adjustment_factor) %>%
  #drop unwanted columns
  select(-c("silt_percent", "adjustment_factor"))
#make emissions table for yes-basement 1-units, then later combine it with the rest
basement_dirt <- make_baseline(efs_table, basement_dusts)


#now that we did the basement dirt displaced, we will do the actual construction emissions
pollutants <- c('PM10-PRI','PM25-PRI','PM10-FIL','PM25-FIL')
#given in TON per ACRE-MONTH. PM25 is assumed to be 10% of PM10
TPPPY <-c(0.011, 0.011*0.1, 0.011, 0.011*0.1)
efs_table <- make_simple_efs_table(scc,pollutants,TPPPY)
#now I need to adjust the EF for each county based on the county_silt_percentages adjustment factors
efs_table <- left_join(efs_table, county_silt_percentages, by="county") %>%
  mutate(TPUPY = TPUPY * adjustment_factor) %>%
  ...
  select(-c("silt_percent", "adjustment_factor"))
#make emissions table for yes-basement 1-units, then later combine it with the rest
yesbasement <- make_baseline(efs_table, single_basement)

#add together emissions from no basements, yes basements, and then the dirt displaced for basements
all_single_unit <- nobasement %>% 
  mutate(TPY = nobasement$TPY + yesbasement$TPY + basement_dirt$TPY)


      #2-family unit construction
#we assume two-family units take up 1/3 of an acre and last 6 months.
two_family_units <- projection_tables[["TwoFamilyUnits"]] %>% filter(year == start_year) %>%
  #we pull new units constructed, so multiply by (1/3*6) to get acre months
  mutate(unit = unit*1/3*6)
#EFs for two-family unit construction are in the NOMAD - you will have to check that these are up to date
pollutants <- c('PM10-PRI','PM25-PRI','PM10-FIL','PM25-FIL')
#given in TON per ACRE-MONTH. PM25 is assumed to be 10% of PM10
TPPPY <-c(0.032, 0.032*0.1, 0.032, 0.032*0.1)
efs_table <- make_simple_efs_table(scc,pollutants,TPPPY)
...
efs_table <- left_join(efs_table, county_silt_percentages, by="county") %>%
  ...
  select(-c("silt_percent", "adjustment_factor"))
#make emissions table for apartments, then later combine it with the rest
two_family_units_table <- make_baseline(efs_table, two_family_units)


      #now combine them
all_res_construction <- apartments_table %>%
  mutate(TPY = apartments_table$TPY + all_single_unit$TPY + two_family_units_table$TPY)
#project and add controls
projected_temp_table <- project_baseline(base_table = all_res_construction, projection_table = projection_tables[["Population"]])

controlled_projected <- add_controls(projected_temp_table)
final_table <- merge_with_final_table(final_table, controlled_projected)

rm(utah_precipitation_evaporation_value, county_silt_percentages, apartments, apartments_table)
rm(basement_pct, single_units, single_nobasement, single_basement, kcubic_yards, basement_dusts, basement_dirt, nobasement, yesbasement, all_single_unit)
rm(two_family_units, two_family_units_table)
rm(scc, pollutants, TPPPY, efs_table, projected_temp_table, controlled_projected, all_res_construction)

#industrial processes > construction: SIC 15-17...
#... road construction > total
#used to replicate WW, now we just pull from it. but we DO add controls
scc <- 2311030000
temp_table <- pull_baseline_from_ww(scc)
projected_temp_table <- project_baseline(temp_table, projection_table = projection_tables[["VMT"]])
#add controls
controlled_projected <- add_controls(projected_temp_table)
final_table <- merge_with_final_table(final_table, controlled_projected)
rm(scc, temp_table, projected_temp_table, controlled_projected)


#done with 23-xx sccs
```

NOTE: For solvents, we always do pt source subtractions AFTER adding controls to the emissions estimates. We assume that our pt source emissions are already using the controls before they get phased in, so we want to subtract [controlled] vs. [controlled]

```{r 24-xx-xxx-xxx: solvents)}
...
# add to final table
final_table <- merge_with_final_table(final_table, temp_table_final)
...
## The emissions factor dropped so much the control is no longer relevant, so temp_table_controlled will have the same TPY values as temp_table_project
temp_table_controlled <- add_controls(raw_proj_data = temp_table_project)
...
# The emissions factor now is a lot smaller than what it was when the control was created, so the control is no longer relevant. We get the same TPY after adding controls.
temp_table_controlled <- add_controls(raw_proj_data = temp_table_project)
...
# There are two relevant rules, but both of them reduce emissions by less than 50%, and our emissions factor has already dropped here by a magnitude of less than the controls, so they are now moot. (No longer relevant.)
temp_table_controlled <- add_controls(raw_proj_data = temp_table_project)
...
# !!!NOTE!!! We should really just use the emission factor from the WW for this one in the 2020 NEI. We are not justified in continuing to use the 10 lb/employee emission factor.
...
final_table <- merge_with_final_table(final_table, temp_table_final)
# remove the objects we created for this scc
rm(scc, temp_table, ww_ef, graphic_arts_efs, temp_table_project, temp_table_controlled, pt_sccs, pt_subs, temp_table_final)
...
# rule R307-357, https://rules.utah.gov/publicat/code/r307/r307-357.htm
# see 2460100000
# add controls, but they don't do anything because the emissions factor is much smaller than what it was when the control was created
temp_table <- add_controls(temp_table)
final_table <- merge_with_final_table(final_table, temp_table)
rm(scc, efs_table, temp_table)
...
controlled_temp_table <- add_controls(temp_table)
#pt subs? Maybe used to be but is no longer supported in EPA crosswalk.
final_table <- merge_with_final_table(final_table,controlled_temp_table)
rm(scc, efs_table, temp_table, controlled_temp_table)
...
# we need to input a current_efs table to the controls function since these emission factors don't exist in the Wagon Wheel. although we converted the above emissions factor to lb/prson, it really is lb/tank. And lb/tank is 0.686, which is the same as what it was when the rule was created. We need to create a tank_efs table with that emissions factor.
tank_efs <- data.frame(FIPS = unique(temp_table_project$FIPS),
                       SCC = scc,
                       pollutant = "VOC",
                       EmissionsFactor = 0.686,
                       EFNumerator = "LB")
...
final_table <- merge_with_final_table(final_table, temp_table_controlled)
...
#done with 24-xx sccs
```

```{r 25-xx-xxx-xxx: storage and transport of fuel}
#some of these SCCs project with the same data: statewide fuel usage. It is the combination of 
...
#####
#these SCCs are calculated in our old 20190502 NEI Workbook, but for now I will pull their numbers from the NEI. I could not understand and track down where the emissions come from. The calculations were too wild. Of all 130+ sccs that I transferred from the workbook into R, these are the only 6 that might suffer a loss of accuracy. NEI methods simply take 1998 data and scale it into our current year based on some proportion. Not idea. If we were to work on some SCCs, these would be a good idea to redo. Good luck. 2021/05/19

#these 6 SCCs were maybe ~4% of area source human-made VOCs. <1% of statewide VOCs.

#pulling these SCCs from the WW gets numbers very different than when pulled from the NEI.
#Even though the WW seems like it would be better to pull from, I don't understand our calculation methods, and our numbers are VERY close to NEI numbers, so I will use the NEI because it most closely reflects our own calculations.


#storage and transport > petroleum and petroleum product storage
#bulk terminals: all evaporative losses > gasoline
scc <- 2501050120
temp_table <- pull_baseline_from_nei(scc)
#temp_table %>% filter(pollutant == "VOC") %>% select(TPY) %>% sum()
#old VOC: 285 TPY
#new VOC: 259 TPY
projected_temp_table <- project_baseline(temp_table, all_gas)
final_table <- merge_with_final_table(final_table, projected_temp_table)
rm(scc, temp_table, projected_temp_table)


#storage and transport > petroleum and petroleum product storage
#gasoline service stations > stage 1: total
#used to be 2501060050. From what I can tell, 2501060053 directly translated to 2501060050
scc <- 2501060053
temp_table <- pull_baseline_from_nei(scc)
#temp_table %>% filter(pollutant == "VOC") %>% select(TPY) %>% sum()
#old VOC: 189 TPY
...
#storage and transport > petroleum and petroleum products ...
# ... > bulk plants: all evaporative losses > gasoline

#EPA has noted that there is a potential for overlap with point sources for SCCs: 2501055120 (likely with Bulk Terminals) and 2505040120
scc <- 2501055120
temp_table <- pull_baseline_from_ww(scc)
...
#done with 25-xx sccs
```

2810010000 - perspiration - report? find citation?
```{r 28-xxx-xxx-xxx: Agriculture, animals, fires}
...
#If we pull nothing, then let's not project/add this scc to the table. This could happen
#if all of the SCCs happen on permitted major pt sites. This would be the case for our WEBSITE
#reporting. As of 20210506, EPA does not offer default numbers for this SCC, so we don't have to
#worry about them filling in any blanks.
if(dim(temp_table)[1]!=0) {
  #project as steady. These are permitted emissions so they may not radically change over time.
  temp_table <-
    project_baseline(temp_table, projection_tables[["Steady"]])
  #no controls
  final_table <- merge_with_final_table(final_table, temp_table)
}
rm(scc, temp_table)
...
# we came up with emissions calculations for this SCC by ourselves. EPA does not offer any methodologies for it. Wheat, Corn, Hay, and Barley are like 99.3% of the particulate matter from harvesting in our state, so those were the crops we focused on.
...
#  1.41E-6 TPPPY
scc <- 2810030000
pollutants <- c('VOC','PM10-PRI','PM25-PRI','CO','NOX','107028','74908','7647010','50000')
...
#  1.16E-7 TPPPY
scc <- 2810050000
pollutants <- c('PM10-PRI','PM25-PRI','VOC','CO','NOX')
...
#misc > health services > dental alloy production > overall process
#previously not in our reporting. Only a very small amount of mercury, but let's add it.
scc <- 2850001000
temp_table <- pull_baseline_from_ww(scc)
temp_table <- project_baseline(temp_table, projection_tables[["Population"]])
#no pt sub
...
temp_table <- add_manual_controls( temp_table, scc,counties = 49003, st_year = 2017, e_year = 2018,end_val = 0.98326)
temp_table <- add_manual_controls( temp_table, scc,counties = 49011, st_year = 2017, e_year = 2018,end_val = 0.98450)
temp_table <- add_manual_controls( temp_table, scc,counties = 49035, st_year = 2017, e_year = 2018,end_val = 0.98781)
temp_table <- add_manual_controls( temp_table, scc,counties = 49045, st_year = 2017, e_year = 2018,end_val = 0.99653)
temp_table <- add_manual_controls( temp_table, scc,counties = 49057, st_year = 2017, e_year = 2018,end_val = 0.99264)
final_table <- merge_with_final_table(final_table,temp_table)
...
temp_table <- pull_baseline_from_ww(scc)
temp_table <- project_baseline(temp_table, projection_tables[["WoodBTU_Residential"]])
temp_table <- add_manual_controls( temp_table, scc,counties = 49003, st_year = 2017, e_year = 2018,end_val = 0.97621)
temp_table <- add_manual_controls( temp_table, scc,counties = 49011, st_year = 2017, e_year = 2018,end_val = 0.98321)
temp_table <- add_manual_controls( temp_table, scc,counties = 49035, st_year = 2017, e_year = 2018,end_val = 0.98817)
temp_table <- add_manual_controls( temp_table, scc,counties = 49045, st_year = 2017, e_year = 2018,end_val = 0.99498)
temp_table <- add_manual_controls( temp_table, scc,counties = 49057, st_year = 2017, e_year = 2018,end_val = 0.99140)
final_table <- merge_with_final_table(final_table,temp_table)
...
temp_table <- pull_baseline_from_ww(scc)
temp_table <- project_baseline(temp_table, projection_tables[["WoodBTU_Residential"]])
temp_table <- add_manual_controls( temp_table, scc,counties = 49003, st_year = 2017, e_year = 2018,end_val = 0.98542)
temp_table <- add_manual_controls( temp_table, scc,counties = 49011, st_year = 2017, e_year = 2018,end_val = 0.98229)
temp_table <- add_manual_controls( temp_table, scc,counties = 49035, st_year = 2017, e_year = 2018,end_val = 0.98526)
temp_table <- add_manual_controls( temp_table, scc,counties = 49045, st_year = 2017, e_year = 2018,end_val = 0.99653)
temp_table <- add_manual_controls( temp_table, scc,counties = 49057, st_year = 2017, e_year = 2018,end_val = 0.99390)
final_table <- merge_with_final_table(final_table,temp_table)
...
temp_table <- pull_baseline_from_ww(scc)
temp_table <- project_baseline(temp_table, projection_tables[["WoodBTU_Residential"]])
temp_table <- add_manual_controls( temp_table, scc,counties = 49003, st_year = 2017, e_year = 2018,end_val = 0.99981)
temp_table <- add_manual_controls( temp_table, scc,counties = 49011, st_year = 2017, e_year = 2018,end_val = 0.99977)
temp_table <- add_manual_controls( temp_table, scc,counties = 49035, st_year = 2017, e_year = 2018,end_val = 0.99981)
temp_table <- add_manual_controls( temp_table, scc,counties = 49045, st_year = 2017, e_year = 2018,end_val = 0.99996)
temp_table <- add_manual_controls( temp_table, scc,counties = 49057, st_year = 2017, e_year = 2018,end_val = 0.99992)
final_table <- merge_with_final_table(final_table,temp_table)
...
temp_table <- pull_baseline_from_ww(scc)
temp_table <- project_baseline(temp_table, projection_tables[["WoodBTU_Residential"]])
temp_table <- add_manual_controls( temp_table, scc,counties = 49003, st_year = 2017, e_year = 2018,end_val = 0.99817)
temp_table <- add_manual_controls( temp_table, scc,counties = 49011, st_year = 2017, e_year = 2018,end_val = 0.99892)
temp_table <- add_manual_controls( temp_table, scc,counties = 49035, st_year = 2017, e_year = 2018,end_val = 0.99939)
temp_table <- add_manual_controls( temp_table, scc,counties = 49045, st_year = 2017, e_year = 2018,end_val = 0.99954)
temp_table <- add_manual_controls( temp_table, scc,counties = 49057, st_year = 2017, e_year = 2018,end_val = 0.99958)
final_table <- merge_with_final_table(final_table,temp_table)
...
#done with 21-xx wood combustion sccs
```

```{r tidy up stuff for submitting for ozone SIP}
#also some QAQC
#there should be 139 sccs now
sort(unique(final_table$SCC))

test <- distinct(final_table)
if(dim(test)[1] != dim(final_table)[1]){
  cat("There are duplicates in final_table somewhere. Maybe use 'duplicated' or something to find it\n")
} else {
  cat("We don't have any duplicates in final_table. Nice.\n")
}

#filter HAPS and write out file
test <- final_table %>% filter(year==2017) %>% 
  filter(pollutant %in% c("CO", "VOC", "NH3", "SO2", "NOX", "PM10-PRI", "PM25-PRI"))
write_csv(test, "CAP_2017SIP.csv")



#####
#2013 compare
all_2013 <- final_table %>% filter(year==2013)
all_2017 <- final_table %>% filter(year==2017)
diff <- compare_inventories(all_2017, all_2013 %>% mutate(year=2017), hide_pct = FALSE)
larger15tpy <- diff %>% filter(abs(tpy_change)>15)
large <- larger15tpy %>% filter(abs(pct_change)>5)
write_csv(large,"large2013vs2017changes.csv")

#####
#compare my 2017 to the old 2017. first we need to filter them down to CAPs
old_2017 <- old_model_data
all_2017 <- final_table %>% filter(year==2017)

fil_new2017 <- all_2017 %>%
  filter(pollutant %in% c("CO", "VOC", "NH3", "SO2", "NOX", "PM10-PRI", "PM25-PRI","PM10","PM25")) %>%
  mutate(pollutant = ifelse(pollutant == "PM10-PRI","PM10", pollutant)) %>%
  mutate(pollutant = ifelse(pollutant == "PM25-PRI","PM25", pollutant))
fil_old2017 <- old_2017 %>%
  filter(pollutant %in% c("CO", "VOC", "NH3", "SO2", "NOX", "PM10", "PM25"))
  
diff <- compare_inventories(fil_old2017, fil_new2017, hide_pct = FALSE)
missing <- diff %>% filter(is.na(tpy_change))
write_csv(missing, "missingFromOld2017.csv")

#####
#2023 compare to 2017
zz <- compare_inventories(all_2017,all_2023 %>% mutate(year=2017), hide_pct = FALSE)
#none are missing. nice
missing <- zz %>% filter(is.na(tpy_change))
greater_than1TPY <- zz %>% filter(abs(tpy_change)>1)
greater_than15TPY <- zz %>% filter(abs(tpy_change)>15)
large_changes <- greater_than15TPY %>% filter(abs(pct_change)>5)
#write out every difference larger than 15 TPY AND larger than 5%
write_csv(large_changes, "largeChangesTo2023.csv")



#####
#filter out 2017 and save it
all_2017 <- final_table %>% filter(year==2017)
write_csv(all_2017, 'AreaOzoneSIP2017_20210526.csv')
#filter out 2023 and save it
all_2023 <- final_table %>% filter(year==2023)
write_csv(all_2023, 'AreaOzoneSIP2023_20210526.csv')
#filter out 2013 and save it
all_2013 <- final_table %>% filter(year==2013)
write_csv(all_2013, 'AreaOzoneSIP2013_20210526.csv')
```
