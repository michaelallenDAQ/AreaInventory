...
## NAA counties
## Note: The EF listed in the Workbook is from the 2014 NEI and is 537 lbs/employee; 
old_control <- 0.73
old_end_ef <- old_control * 537 
#let's see what the new EF is, and adjust our controls to reach the right end EF.
#Pull the VOC EF from WW
new_ef <- pull_efs_from_ww(scc, throughput_unit = "EACH", all_primaries = FALSE) %>% filter(pollutant == "VOC") %>% select(TPUPY) %>% unique() %>% head() %>% as.double()*2000
#get a new end control value. It is simply the difference between our old final value and our new EF.
new_control <- (old_end_ef/new_ef)
if(new_control< old_control) {
  new_control <- old_control
...
#the code will break if we try to project a blank table
if(dim(pt_subs)[1]!=0){
...
## NAA counties
## Note: The EF listed in the Workbook is from the 2014 NEI and is 415 lbs/employee
old_control <- 0.29
old_end_ef <- old_control * 415
#let's see what the new EF is, and adjust our controls to reach the right end EF.
#Pull the VOC EF from WW
new_ef <- pull_efs_from_ww(scc, throughput_unit = "EACH", all_primaries = FALSE) %>% filter(pollutant == "VOC") %>% select(TPUPY) %>% unique() %>% head() %>% as.double()*2000
#get a new end control value. It is simply the difference between our old final value and our new EF.
