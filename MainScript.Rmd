...
old_control <- 0.27
old_end_ef <- old_control * 87 #23.49
...
#get a new end control value. It is simply the difference between our old final value and our new EF.
new_control <- (old_end_ef/new_ef)
#if the new control value has DECREASED from the old value, that means the EF has gone up since
#it was last updated, and we need a more extreme control factor.
#If that's the case, we will assume the increased EF is real, and we will
#not add more aggressive controls. So if the new control value has become smaller/more aggressive,
#reset it to the old value.
if(new_control< old_control) {
  new_control <- old_control
}
#if the new EF is so small that it renders our old control factor useless, we will get a control
#factor greater than 1. If this is the case, just set the control to 1 (will not affect emissions)
if(new_control >1){
  new_control <- 1
}
...
temp_table_controlled <- add_controls(raw_proj_data = temp_table_project, sccs = scc, counties = naa_counties, st_year = 2017, e_year = 2022, end_val = new_control)
...
old_control <- 0.26
old_end_ef <- old_control * 437 
#let's see what the new EF is, and adjust our controls to reach the right end EF.
#Pull the VOC EF from WW
new_ef <- pull_efs_from_ww(scc, throughput_unit = "EACH", all_primaries = TRUE) %>% filter(pollutant == "VOC") %>% select(TPUPY) %>% unique() %>% head() %>% as.double()*2000
#get a new end control value. It is simply the difference between our old final value and our new EF.
new_control <- (old_end_ef/new_ef)
if(new_control< old_control) {
  new_control <- old_control
