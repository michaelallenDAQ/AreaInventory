...
#subtract pt values from total. The 'unit' column is still the statewide value of area coal, 
#meaning we need to distribute the values on a per-county basis
area_coal$unit <- total_coal$unit - pt_coal$unit
#turn negative values to 0
area_coal$unit[area_coal$unit<0] <-0
...
#subtract pt values from total. The 'unit' column is still the statewide value of area kerosene,
#meaning we need to distribute the values on a per-county basis
area_kerosene$unit <- total_kerosene$unit - pt_kerosene$unit
#turn negative values to 0
area_kerosene$unit[area_kerosene$unit<0] <-0
#adjust KBBL throughput into KGal
area_kerosene$unit <- area_kerosene$unit * 42
...
#1) This is a list of landfill sites contained in the DAQ database. Several of these landfills submit inventories. The listing of landfills and inventories can be found in relevent appendix of the Input Appendices.
#2) Counties with municipal landfills was derived from a list from the Utah Division of Waste Management  (point of contact: Allan Moore, allanmoore@utah.gov or 536-0211) and is available in relevent appendix of the Input Appendices. Each county's tons of waste was also adjusted using this dataset to remove tons of waste from the point source landfills present in each county (avoid double count with point source landfill emission submission).									
#3) Emissions are included from current inventories.									
#4) Information from the Division of Waste Management shows that every county has a municipal landfill except Morgan and Wasatch.									
#5) It is assumed that Landfill PM includes the act of daily soil coverage (see below notes on landfill emission factors as well).									
#6) Landfill VOC, NH3, PM10, and PM2.5 emission factors were calculated based on inventoried point source landfills emissions per ton of waste accepted in 2014, then projected to other landfills based on the waste accepted by the landfills in each county.									
#7) Point landfill emissions are derived from data in Form 6a or Form 11b of their inventory submission to Utah DAQ (Scott Hanks or Deb McMurtrie) or adjusted from summaries to remove PM10/2.5 generated from haul roads, engines etc (accounted for in "Unpaved Roads" and combustion categories) and limited to emissions from landfill operations (eg Wasatch Integrated Waste's incinderator emissions are removed). Note: submitted inventories must be used as Tempo summaries do not provide the detail required to limit emissions to soil covering operations. VOCs are adjusted by excluding tailpipe and other non-landfill emission sources. In the future, it would be better to be able to use inventory data (if available) to create an average of emission/ton waste rather than using a single year's data.									
#8) The above factors will be used in conjunction with individual county landfill tons of waste accepted to calculate uninventoried landfill emissions.									
#9) The "1997 Gridded Ammonia Emission Inventory, Update for the South Coast Air Basin," by Charles W. Botsford, et al., page A-43, proposes a correlation of 0.007 to 1 ammonia to methane.  For this use, methane and VOC are assumed equal.									

scc <- 2620030000


#efs were pulled from looking at permitted landfills. We took their throughputs and emissions and back-calculated to find the throughputs. For documentation, look in 
