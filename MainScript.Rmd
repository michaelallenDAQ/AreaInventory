...
pt_kerosene <- projection_tables[["PtKerosene_Kerosene, Industrial"]] %>% filter(year == start_year)
...
pt_kerosene <- projection_tables[["PtKerosene_Kerosene, Commercial"]] %>% filter(year == start_year)
#set up area kerosene with the same format as total, but then subtract pt_kerosene from it
area_kerosene <- total_kerosene
#subtract pt values from total. The 'unit' column is still the statewide value of area kerosene,
#meaning we need to distribute the values on a per-county basis
...
pt_lpg <- projection_tables[["PtLPG_LPG, Industrial"]] %>% filter(year == start_year)
...
pt_lpg <- projection_tables[["PtLPG_LPG, Commercial"]] %>% filter(year == start_year)
#subtract pt from total to get area
area_lpg <- all_lpg
area_lpg$unit <- all_lpg$unit - pt_lpg$unit
#turn negative values to 0
...
nonresNG <- projection_tables[["NonResNGMMCF_All State, UTAH"]] %>% filter(year == start_year)
...
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["NonResNGMMCF_All State, UTAH"]])
...
resNGMMCF <- projection_tables[["ResNGMMCF_All State, UTAH"]] %>% filter(year == start_year)
...
projected_temp_table <- project_baseline(base_table = temp_table, projection_table = projection_tables[["ResNGMMCF_All State, UTAH"]])
#add controls
controlled_projected <- add_controls(projected_temp_table)
