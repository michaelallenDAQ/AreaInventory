...
#Control factor was based on 2014 which assumed 87 lbs/employee.
# 73% reduction on 87 lbs would end with 23.49 lb/employee.
old_control <- 0.27
old_end_ef <- 23.49
#let's see what the new EF is, and adjust our controls to reach the right end EF.

#Pull the VOC EF from WW
new_ef <- pull_efs_from_ww(scc, throughput_unit = "EACH") %>% filter(pollutant == "VOC") %>% select(TPUPY) %>% unique() %>% head() %>% as.double()*2000
#get a new end control value. It is simply the different between our old final value and our new EF.
new_control <- (old_end_ef/new_ef)
#if the new control value has INCREASED from the old value, that means the EF has gone up since
#it was last updated. If that's the case, we will assume the increased EF is real, and we will
#not add more aggressive controls. So if the new control value has become larger/more aggressive,
#reset it to the old value.
if(new_control< old_control) {
  new_control <- old_control
}
temp_table_controlled <- add_controls(raw_proj_data = temp_table_project, sccs = scc, counties = naa_counties, st_year = 2015, e_year = 2020, end_val = new_control)
...
#
#for now, just use WW, as at least the throughput and EF are synced up. 
scc <- 2401008000
