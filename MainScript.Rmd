...
# Natural gas consumption is reported directly by gas suppliers on the county level.								
# Questar's Dave Landward (dave.landward@questar.com or 801-324-5722) provided Questar's 2014 usage consumption proportions by sector (1/27/16 email) used to distribute non-residential natural gas usage. Proportions were: Residential - 36%, Commercial - 22%, Industrial - 19% and Electric - 23%.  The residential proportion was removed and non-residential proportions adjusted accordingly to determine each sector's proportion of non-residential MMCF. For example, Commercial proportion =  Commercial% / (Commercial%+Industrial%+Electric%) = 22%/(22%+19%+23%) = 30%.										
...
# Note: Questar does not track cubic feet. Rather, they convert decatherms to cubic feet by a current system BTU average of 1036 per cubic foot (see Questar data source in the relevent Input Appendix of the input table workbook). Be aware that this average may change over time based on their gas source characteristics.										
...
#Change NH3 to 3.2 per Rich Mason! OR Note: (10/16/17) Ammonia emission factor was changed to zero based on industry feedback (Frank Stanonick, Air Conditioning, Heating & Refrigeration Institute call with Joel Karmazyn)  which also reflects that AP42 has no emission factor for ammonia.	
...
#now get throughput. Start by pulling the Electrical/Industrial/Commercial NG use for all of Utah. Basically all of the nonresidential NG use in Utah. Units MMCF
nonresNG <- projection_tables[["NonResNGMMCF_All State, UTAH"]] %>% filter(year == start_year)
#now we break out the industrial portion of the statewide gas usage.
#Based on correspondence we had with Dave Landward of Questar Gas (Now Dominion Energy) in 2014, NG use in the state was as follows:
#   Residential:  36%
#   Commercial:   22%
#   Industrial:   19%
#   Electric:     23%
